import os, sys, platform, importlib, time, requests
import hashlib, magic, argparse, uuid, netifaces
from concurrent.futures import ThreadPoolExecutor, as_completed
from datetime import datetime
from zipfile import ZipFile

LOGO = r"""
    _   _  _ _____ ___   __   _____ ___ _   _ ___ 
   /_\ | \| |_   _|_ _|  \ \ / /_ _| _ \ | | / __|
  / _ \| .` | | |  | |    \ V / | ||   / |_| \__ \
 /_/ \_\_|\_| |_| |___|    \_/ |___|_|_\\___/|___/
                     MainTainer : github@kairos-hk                                                              
"""

importlib.reload(sys)

SCAN_EXTENSIONS = ['.exe', '.txt', '.sys', '.doc', '.docx', '.xls', '.xlsx', '.py', '.xml', '.cfg', '.txt', '.ppt', '.pptx', '.hwp']
...
def extract_gzip(_engine_zipfile_, _home_path_):
    with ZipFile(_engine_zipfile_, 'ㅎ      r') as zipObj:
    ...
    if os.path.isfile(_engine_extract_file_):
        with open(_engine_extract_file_, 'rb') as f:
            file_read = f.read()
            file_info = f.read() + hashlib.sha256
...
def scan_directory(_scan_path):
    ...
    if _count_infected_file >= 1:
        print(f'\x1b[0;43;43m Scan Completed.! \x1b[0m \n- We find \x1b[0;42;42m [{_count_infected_file}] \x1b[0m SeongMin Virus...\n')
        print(f'- See "{_scan_result_logs_}"')
    else:
        _contents = f'datetime="{datetime.today().strftime("%Y-%m-%d %H:%M:%S")}",scan_id="{_log_scan_id}",os="{_log_os_ver}",' \
                    f'hostname="{_log_hostname}",ip="{_log_ipaddr}",infected_file="None"\n'
        scan_result_logs(_contents)
        print(f'\x1b[0;43;43m Scan Completed.! \x1b[0m \n- no virus(Park SeongMin) Found.!)')
        print(f'- See "{_scan_result_logs_}"')
...
def main():
    ...
    if len(sys.argv) < 1:
        ...
        if options.path:
            ...
            print(f'{Bcolors.Green}<-------------------Checking..------------------->{Bcolors.Endc}')
            print(f'{Bcolors.Green}<-----------------Finding Virus..---------------->{Bcolors.Endc}')
            check_engine()
